...
<DiagramaFlujo
  alt="Flujo: el servidor de correo dispara un trigger en N8N, que extrae remitente y asunto, hace una petición HTTP a Evolution API, y esta envía el mensaje al grupo de WhatsApp del equipo."
  pasos={[
    { icono: "correo", lineas: ["SERVIDOR", "CORREO (IMAP)"] },
    { icono: "reloj", lineas: ["N8N", "TRIGGER"] },
    { icono: "http", lineas: ["N8N HTTP", "REQUEST"] },
    { icono: "chat", lineas: ["EVOLUTION API", "→ WHATSAPP"] },
  ]}
  flechas={["NUEVO EMAIL", "REMITENTE + ASUNTO", "POST"]}
  rama={{ desde: 2, lineas: ["GRUPO WHATSAPP", "DEL EQUIPO"], etiqueta: "ENVÍA MENSAJE" }}
/>
...
**HTTPS.** Para producción, lo ideal es poner un proxy inverso (Nginx o Caddy) delante de los servicios con un certificado TLS. Caddy es la opción más cómoda: gestiona los certificados de Let's Encrypt de forma automática y sin configuración extra.
