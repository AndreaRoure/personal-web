...
2. Consulta el BOE
3. Analiza los resultados: filtra por territorio (Madrid, Cataluña, Estatal, UE), descarta lo irrelevante, puntúa lo que sí encaja con la misión de Exit
...
**Hermes (Python + LLM vía OpenRouter)** es el que piensa. Usa un modelo de lenguaje (Qwen3.7-flash) para leer los datos, evaluar qué es relevante para Exit y redactar el briefing. No ejecuta llamadas a APIs directamente — solo decide qué hacer.
...
*Hermes no es un proyecto de IA complejo. Es un agente sencillo con tres herramientas bien definidas, sin ningún framework de agentes, que resuelve un problema concreto: que el equipo de Exit empiece el día sabiendo qué convocatorias ha publicado el BOE esta mañana. Eso antes no pasaba. Ahora sí.*
